...
> There is no `@eudi-verify/php` package. Two integration paths are available: choose based on your deployment constraints. For the full platform support matrix, see [SUPPORTED.md](./SUPPORTED.md). For sequence diagrams of the same flow, see [integration-architecture.md](./integration-architecture.md#production-flow-php-proxy--node-sidecar).
...
For a full server example see [INTEGRATION.md](./INTEGRATION.md) or [examples/server](../examples/server/).
...
| `POST /api/eudi/callback`            | EUDI Wallet      | Receives the wallet's `direct_post` response      |
| `GET /api/eudi/request/:id`          | EUDI Wallet      | Serves the OpenID4VP authorization request object |

The `/callback` and `/request/:id` endpoints are called by the EUDI Wallet itself, not by your frontend, and require implementing the [OpenID4VP](https://openid.net/specs/openid-4-verifiable-presentations-1_0.html) protocol, including mdoc/VP verification. No pre-built PHP library for the EUDI protocol is available yet: this is a significant undertaking.
...
## Verification engine and demo mode

Both paths use whichever engine your Node sidecar (Path A) or PHP implementation (Path B) is wired to:

- **Demo engine** (`OpenEudiEngine` with `mode: "demo"`): simulated claims only, limited to `age_over_18` and country/nationality. No real identity verification. All responses carry `X-Eudi-Mode: demo`.
- **Production engine** (`Openid4vpEngine`, real mdoc verification for `eu.europa.ec.av.1` age attestation): validated in the lab against the EU Age Verification reference wallet. See [SUPPORTED.md](./SUPPORTED.md) and [INTEROP.md](./INTEROP.md) for exact coverage and trust-store requirements before relying on it for real identity decisions.

If a PHP checkout handler needs to branch on mode, read the `X-Eudi-Mode` response header rather than hardcoding an assumption.
...
- [integration-architecture.md](./integration-architecture.md) — sequence diagrams for the PHP proxy + Node sidecar flow
