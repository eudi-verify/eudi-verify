...
const TERMINAL = new Set(['verified', 'rejected', 'expired', 'cancelled', 'error']);

async function loadSession() {
  if (!sessionId) {
    showError('Missing session ID. Open this page from the verification demo (link includes ?state=…).');
    return;
  }
  ...
  if (!res.ok) {
    showError(res.status === 404 ? 'Session not found or expired.' : `Failed to load session (${res.status}).`);
    log(`→ ${res.status}`);
    return;
  }
  ...
  requestEl.textContent =
    fromAuth ?? formatClaims(session.request) ?? '—';

  if (TERMINAL.has(session.status)) {
    actionsEl.hidden = true;
    doneEl.hidden = false;
    document.getElementById('wallet-done-title').textContent =
      session.status === 'verified' ? 'Already approved' : `Session ${session.status}`;
...
async function approve() {
  ...
  document.getElementById('session-status').textContent = 'verified (pending poll)';
}
