...
describe('render', () => {
  describe('getStateId', () => {
    it('returns prefixed id for each status', () => {
      ...
      expect(getStateId('waitingForWallet')).toBe('eudi-state-waitingForWallet');
  ...
  describe('renderWidget', () => {
    ...
    it('idle state is active by default', () => {
      const html = renderWidget();
      expect(html).toContain('id="eudi-state-idle" class="eudi-state" data-active');
    });
  });
  ...
  describe('updateWidgetState', () => {
    ...
    it('activates error state and shows message', () => {
      ...
      const detail = container.querySelector('#eudi-state-error .eudi-error-detail');
    ...
    it('only one state is active at a time', () => {
      ...
      for (const state of states) {
        ...
        const activeStates = container.querySelectorAll('.eudi-state[data-active]');
